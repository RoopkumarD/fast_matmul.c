...
if __name__ == "__main__":
    ...
    MAT_SIZES = np.linspace(MIN_SIZE, MAX_SIZE, NUM_PTS, endpoint=True, dtype=int)
    avg_flops = []
    min_flops = []
    max_flops = []
    ...
    for SIZE in tqdm(MAT_SIZES, desc="Benchmark"):
        ...
        min_flops.append(FLOP / min_exec_time)
        max_flops.append(FLOP / max_exec_time)
    ...
    if SHOW_FIG or SAVE_FIG:
        ...
        ax.set_title(f"NumPy(=OpenBLAS) {title_ncores}", fontsize=18)
